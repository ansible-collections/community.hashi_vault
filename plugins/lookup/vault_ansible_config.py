...
class LookupModule(HashiVaultLookupBase):
    def run(self, terms, variables=None, **kwargs):
        ...
        # vr = loader.lookup_loader('community.hashi_vault.vault_ansible_config')

        rd = {}
        ret = [rd]

        pluginget = C.config.get_plugin_options('lookup', 'ansible_collections.community.hashi_vault.plugins.lookup.vault_ansible_config')

        for k, v in pluginget.items():
            if k[0] != '_':
                v, o = C.config.get_config_value_and_origin(k, None, 'lookup', 'ansible_collections.community.hashi_vault.plugins.lookup.vault_ansible_config', None, variables=variables)
                if v is not None and o != 'default' and True:
                    rd[k] = (v, o)

        # ret = [
        #     dict(

        #         u = C.config.get_config_value_and_origin('url', None, 'lookup', 'ansible_collections.community.hashi_vault.plugins.lookup.vault_ansible_config'),
        #         w = C.config.get_config_value_and_origin('wrap_ttl', None, 'lookup', 'ansible_collections.community.hashi_vault.plugins.lookup.vault_ansible_config', None, variables=variables),
        #     )
        # ]
