...
class TestModuleVaultDatabaseConnectionsList():
    ...
    def test_vault_database_connections_list_authentication_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    @pytest.mark.parametrize('exc', [HashiVaultValueError('throwaway msg'), NotImplementedError('throwaway msg')])
    def test_vault_database_connections_list_auth_validation_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    def test_vault_database_connections_list_return_data(self, patch_ansible_module, list_response, vault_client, capfd):
        client = vault_client
        ...
        client.secrets.database.list_connections.assert_called_once_with(mount_point=patch_ansible_module['engine_mount_point'])
        ...
        assert result['connections'] == roles, "module result did not match expected result:\nexpected: %r\ngot: %r" % (list_response, result)

    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    def test_vault_database_connections_list_no_data(self, patch_ansible_module, vault_client, capfd):
        client = vault_client
        raw = {"errors": []}
    ...
    def test_vault_database_connections_list_no_hvac(self, capfd):
        with mock.patch.multiple(vault_database_connections_list, HAS_HVAC=False, HVAC_IMPORT_ERROR=None, create=True):
            with pytest.raises(SystemExit) as e:
                vault_database_connections_list.main()
    ...
    def test_vault_database_connections_list_vault_exception(self, vault_client, exc, opt_engine_mount_point, capfd):
