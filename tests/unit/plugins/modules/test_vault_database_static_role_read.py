...
class TestModuleVaultDatabaseStaticRoleRead():
    ...
    def test_vault_database_static_role_read_authentication_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    @pytest.mark.parametrize('exc', [HashiVaultValueError('throwaway msg'), NotImplementedError('throwaway msg')])
    def test_vault_database_static_role_read_auth_validation_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    def test_vault_database_static_role_read_return_data(self, patch_ansible_module, list_response, vault_client, capfd):
        client = vault_client
        ...
        client.secrets.database.read_static_role.assert_called_once_with(mount_point=patch_ansible_module['engine_mount_point'], name=patch_ansible_module['role_name'])
    ...
    def test_vault_database_static_role_read_no_hvac(self, capfd):
        with mock.patch.multiple(vault_database_static_role_read, HAS_HVAC=False, HVAC_IMPORT_ERROR=None, create=True):
            with pytest.raises(SystemExit) as e:
                vault_database_static_role_read.main()
    ...
    def test_vault_database_static_role_read_vault_exception(self, vault_client, exc, opt_engine_mount_point, capfd):
