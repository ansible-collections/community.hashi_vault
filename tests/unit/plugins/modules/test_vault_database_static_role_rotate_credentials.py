...
class TestModuleVaultDatabaseStaticRoleRotateCredentials():
    ...
    def test_vault_database_static_role_rotate_credentials_authentication_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    @pytest.mark.parametrize('exc', [HashiVaultValueError('throwaway msg'), NotImplementedError('throwaway msg')])
    def test_vault_database_static_role_rotate_credentials_auth_validation_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    def test_vault_database_static_role_rotate_credentials_return_data(self, patch_ansible_module, list_response, vault_client, capfd):
        client = vault_client
        client.secrets.database.rotate_static_role_credentials.return_value = response_obj()
        ...
        client.secrets.database.rotate_static_role_credentials.assert_called_once_with(mount_point=patch_ansible_module['engine_mount_point'], name=patch_ansible_module['role_name'])

        raw = list_response.copy()
        data = raw['data']
        assert result['data'] == data, "module result did not match expected result:\nexpected: %r\ngot: %r" % (list_response, result)


    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    def test_vault_database_static_role_rotate_credentials_no_hvac(self, capfd):
        with mock.patch.multiple(vault_database_static_role_rotate_credentials, HAS_HVAC=False, HVAC_IMPORT_ERROR=None, create=True):
            with pytest.raises(SystemExit) as e:
                vault_database_static_role_rotate_credentials.main()
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (hvac.exceptions.InvalidPath, "", r"^Invalid or missing path \['([^']+)/rotate-role/([^']+)'\]"),
        ]
    )
    ...
    def test_vault_database_static_role_rotate_credentials_vault_exception(self, vault_client, exc, opt_engine_mount_point, capfd):

        client = vault_client
        client.secrets.database.rotate_static_role_credentials.side_effect = exc[0](exc[1])
