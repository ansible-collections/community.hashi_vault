...
class TestModuleVaultDatabaseConnectionConfigure():
    ...
    def test_vault_database_connection_configure_authentication_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    @pytest.mark.parametrize('exc', [HashiVaultValueError('throwaway msg'), NotImplementedError('throwaway msg')])
    def test_vault_database_connection_configure_auth_validation_error(self, authenticator, exc, capfd):
    ...
    @pytest.mark.parametrize('patch_ansible_module', [_combined_options()], indirect=True)
    def test_vault_database_connection_configure_return_data(self, patch_ansible_module, list_response, vault_client, capfd):
        client = vault_client
    ...
    def test_vault_database_connection_configure_no_hvac(self, capfd):
        with mock.patch.multiple(vault_database_connection_configure, HAS_HVAC=False, HVAC_IMPORT_ERROR=None, create=True):
            with pytest.raises(SystemExit) as e:
                vault_database_connection_configure.main()
    ...
    def test_vault_database_connection_configure_vault_exception(self, vault_client, exc, opt_engine_mount_point, capfd):
